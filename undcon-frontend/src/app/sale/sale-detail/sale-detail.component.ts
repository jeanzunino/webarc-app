...
export class SaleDetailComponent implements OnInit, OnDestroy {
  ...
  public getTitle() {
    if (!this.isNew()) {
      const status = this.translate.instant(`enums.sale-status.${this.entity.status.toUpperCase()}`);
  ...
  isTotalBilled() {
    return this.isNew() ? false : this.entity.status === SaleStatus.TOTAL_BILLED;
  }
